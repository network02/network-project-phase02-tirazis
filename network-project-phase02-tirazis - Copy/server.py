...
commands_list = [
    ...
    ("PREVIEW\t", "Preview a file: PREVIEW <filename>"),
...
class FTPthread(threading.Thread):
    ...
    def STOR(self, username, name):
        ...
        while len(received_data) < file_size:
            ...
            received_data += data
            print(f"Received {len(received_data)} of {file_size} bytes")  # Print the progress
            # time.sleep(0.01)  # Add a delay after each chunk
        with open(name, 'wb') as file:
            file.write(received_data)
        self.log_action(username, f"STOR: f{name}")
    ...
    def PREVIEW(self, filename):
        try:
            with open(filename, 'r') as file:
                # Send the first 5 lines of the file
                for i in range(5):
                    line = file.readline()
            ...
            self.client.send(f"An error occurred: {e}".encode())
    ...
    def RETR(self, username, filename):
        ...
        # Send the file data in chunks, and print the progress
        total_bytes_sent = 0
        for i in range(0, len(file_data), 1024):
            data_chunk = file_data[i:i + 1024]
            self.client.sendall(data_chunk)
            total_bytes_sent += len(data_chunk)
            print(f"Sent {total_bytes_sent} of {file_size} bytes")
            # time.sleep(0.01)  # Add a delay after each chunk
        self.client.recv(1024)
        self.log_action(username, f"RETR: f{filename}")
        self.client.send("226 Transfer complete\r\n".encode())
    ...
    def CWD(self, username, dir_name):
        ...
        try:
            ...
            self.log_action(username, f"CWD: f{dir_name}")
        except FileNotFoundError:
    ...
    def CDUP(self, username):
        ...
        try:
            ...
            self.log_action(username, f"CDUP:")
        except FileNotFoundError:
